...
#  Get list of removed test functions from file changes [javaparser]
def get_removed_test_functions(file_changes: str, file) -> List:
    if not file.testcases_before or len(file.testcases_before)==0:
        return []
    
    if not file.testcases or len(file.testcases) == 0:
        return file.testcases_before
    
    removed_methods = []
    for tc in file.testcases_before:
        if tc not in file.testcases:
            removed_methods.append(tc)
...
#  Get list of added test functions from file changes [javaparser]
def get_added_test_functions(file_changes: str, file) -> List:
    ...
    added_methods = []
    for tc in file.testcases:
        if tc not in file.testcases_before:
            added_methods.append(tc)
